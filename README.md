...
Фронтенд донат-магазина Minecraft-сервера SilverLand: каталог товаров, корзина, промокоды и скидки, личный кабинет и полноценная админка для управления всем этим без деплоя.

![Главная страница SilverLand](docs/main-page.png)

Это пет-проект, но собран он как рабочий: строгий TypeScript, архитектурные правила, которые проверяет линтер, визуальные регрессионные тесты и автогенерация типов из бэкендовой OpenAPI-спеки. Руками здесь синхронизируется минимум.
...
Правила композиции слоёв не живут в вики, о которой все забыли: их проверяет ESLint, а dependency-cruiser строит граф зависимостей, по которому видно любое нарушение. Импорты идут через алиасы `@/app/*`, `@/entities/*`, `@/features/*`, `@/pages/*`, `@/shared/*`, `@/widgets/*`.

## Стек

React 18 + TypeScript 5.3, сборка на Vite 6. Состояние на MobX, роутинг на React Router 6, запросы через Axios, стили на Sass.
...
Та часть, ради которой проект приятно показывать.
